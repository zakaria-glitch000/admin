    ...
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="fw-bold m-0 text-dark">Créer un Nouveau Rôle</h2>
                    <p class="text-muted small m-0">Définissez le nom du rôle et attribuez-lui des permissions.</p>
                </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('roles.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control" placeholder="Ex: Technicien, Commercial, Admin..." required>
                        </div>
                                                        ...
                                                        @if(in_array($value->name, $groupPerms))
                                                            <div class="col-md-4 col-sm-6">
                                                                <div class="form-check border rounded p-2 ps-4">
                                                                    <input class="form-check-input permission-checkbox" type="checkbox" name="permission[{{ $value->id }}]" value="{{ $value->id }}" id="perm-{{ $value->id }}"
                                                                        {{ (is_array(old('permission')) && array_key_exists($value->id, old('permission'))) ? 'checked' : '' }}>
                        ...
                        <div class="text-end mt-4 pt-3 border-top">
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="bx bx-save me-1"></i> Enregistrer le Rôle
                            </button>
                        </div>
                    </form>
                </div>
